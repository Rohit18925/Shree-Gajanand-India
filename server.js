...
function respondError(req, res, status, message) {
  if (wantsJson(req)) {
    return res.status(status).json({ result: 'error', error: message });
  }
...
async function sendWithResend({ to, replyTo, subject, text }) {
...
app.post('/submit-form', contactRateLimit, async (req, res) => {
  ...
  const recipient =
    ...
    'shreegajanandindia751@gmail.com';
  ...
  try {
    ...
    return respondError(
      ...
      res,
      500,
      'We could not send your enquiry right now. Please try again or contact us by phone/WhatsApp.'
    );
  }
});
